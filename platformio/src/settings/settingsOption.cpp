...
int SettingsOptionInt::change(int dir)
{
	...
	*setting_ref = current;
	settings.save(true);
	// Serial.println("int: "+String(*setting_ref));
	return *setting_ref;
}
...
// HEXCOLOR
//
void SettingsOptionColor565::change(uint16_t val)
{
	*setting_ref = val;
	settings.save(true);
}

bool SettingsOptionColor565::update(uint16_t val)
{
	if (*setting_ref == val)
		return false;
...
uint16_t SettingsOptionColor565::get() { return *setting_ref; }
...
String SettingsOptionColor565::generate_html(uint16_t index)
{
	...
	String html = "					<div class='input-group input-group-sm'>\n";
	html += "						<span class='input-group-text' id='inputGroup-sizing-sm'>" + fieldname + "</span>\n";
	html += "						<input type='color' class='form-control form-control-sm' id='" + fn + "' name='" + fn + "' value='" + settings.color565ToWebHex(get()) + "'" + (required_field ? "required" : "") + ">\n";
	// html += "					<span>0x" + String(get()) + "</span>&nbsp;<span>" + settings.color565ToWebHex(get()) + "</span>\n";
	html += "					</div>\n";
...
// FLOAT RANGE
//
float SettingsOptionFloatRange::change(int dir)
{
	int current = *setting_ref;
	current += (dir * value_step);
	if (value_wrap)
...
bool SettingsOptionFloatRange::update(float val)
{
	if (*setting_ref == val)
		return false;
...
float SettingsOptionFloat::change(int dir)
{
	...
	// Serial.println("float: "+String(*setting_ref));
	*setting_ref = current;
	settings.save(true);
	return *setting_ref;
}
...
bool SettingsOptionBool::change()
{
	*setting_ref = !*setting_ref;
	...
	settings.save(true);
	// Serial.println("bool: " + String(*setting_ref));
	return *setting_ref;
}
